...
Install [Stack Stats 0.3.0 from VSIX](apps/vscode-extension/stack-stats-vscode-0.3.0.vsix) using **Extensions → … → Install from VSIX…**, then reload if prompted. No CLI, account, or daemon is required. Tracking starts automatically on eligible edits.

Select the Stack Stats stack icon in the Activity Bar for seven native views: **Current Session**, **Today**, **This Week**, **Languages**, **Projects**, **Streak**, and **Tracking Status**. Current Session shows estimated active time, files edited, lines added/removed, edits, and timestamps. Today/This Week show existing daily/weekly totals; This Week adds longest session, active days, and average active time. Languages and Projects rank this week's activity, with expandable edit/line counts. Streak includes historical days across week boundaries. Tracking Status includes storage/history health and pause/resume/settings actions.
...
10. Select the Stack Stats Activity Bar icon and inspect all seven sections. Type in a visible editor: counts should update within about one second without Refresh Stats. Expand language/project rows; check empty states in a fresh profile. Click the status bar and verify Current Session opens.
